...
2. **Select and segment** the transcript. Claude reads the call and decides which
   ...
   [`selecting-clips.md`](skills/coaching-call-clipper/references/selecting-clips.md).
...
Then copy the folder `skills/coaching-call-clipper` into your Claude Code skills
directory:
...
1.5 GB). You can let the skill do this automatically, or build it ahead of time:

```
# Windows (PowerShell)
skills\coaching-call-clipper\setup.ps1

# macOS / Linux
bash skills/coaching-call-clipper/setup.sh
```
...
stages yourself; see [the skill's instructions](skills/coaching-call-clipper/SKILL.md).
...
```
<venv-python> skills/coaching-call-clipper/scripts/batch_export.py <calls-dir> --pad 180
```
...
├── skills/
│   └── coaching-call-clipper/
│       ├── SKILL.md           the skill: the full workflow Claude follows
│       ├── references/
